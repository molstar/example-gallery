...
function toggleWater(plugin: PluginContext, hide: boolean){
    // Find all structure components with the water structure component tag
    const waterComponentTag = 'structure-component-static-water';   // Tag generated by StructureRepresentationPresetProvider
    const structComponentRef = StateSelection.findTagInSubtree(
        plugin.state.data.tree,                         // The state tree
        plugin.state.data.tree.root.ref,                // The root ref of the state tree
        waterComponentTag                               // The tag to search for
    );
    if (!structComponentRef) return;    // if no structure components are found, return

    // Set the visibility of each structure component and any StateObjects in its subtree (including representations)
    setSubtreeVisibility(plugin.state.data, structComponentRef, hide)
}